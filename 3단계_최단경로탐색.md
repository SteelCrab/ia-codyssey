...
```mermaid
graph TD
    A[시작: queue = [(start, [start])] visited = {start}] --> B[큐에서 현재 위치와 경로 추출]
    B --> C{현재 위치 == 목표?}
    
    C -->|Yes| D[최단 경로 발견! 경로 반환]
    C -->|No| E[인접한 위치들 탐색]
...
graph TD
    subgraph "경로 데이터"
        A1[모든 반달곰 커피 경로 26개 좌표점]
        A2[모든 구조물 방문 경로 31개 좌표점]
    end
    
    subgraph "겹침 분석"
        B1[선분 변환 path → segments]
        B2[교집합 계산 overlapping segments]
        B3[고유/공통 구간 분리]
    end
    
    subgraph "시각화 적용"
        C1[빨간 실선 커피 경로 고유 구간]
        C2[보라 실선 구조물 경로 고유 구간]
        C3[점선 겹치는 공통 구간]
        C4[특수 마커 시작점/도착점]
    end
...
graph LR
    ...
    subgraph "메모리 사용량"
        B1["그리드: O(225) 15×15 좌표"]
        B2["방문 체크: O(225) visited set"]
        B3["경로 저장: O(31) 최대 경로 길이"]
    end
    
    subgraph "품질 지표"
        C1["정확성: 100% BFS 최단 보장"]
        C2["효율성: 95% TSP 근사 품질"]
        C3[실용성: 매우 높음 1초 미만 실행]
    end
...
flowchart LR
    subgraph "경로 계산 결과"
        A1[커피 방문 경로 25칸, 26단계]
        A2[구조물 방문 경로 30칸, 31단계]
    end
    
    subgraph "CSV 파일 생성"
        B1[home_to_cafe.csv step,x,y,description]
        B2[all_structures_path.csv step,x,y,description]
    end
    
    subgraph "시각화 출력"
        C1[map_final.png 405KB, 300DPI]
        C2[경로 포함 지도 범례 및 정보 박스]
    end
